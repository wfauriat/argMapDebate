...
function PolicyNode({ id, data, selected }: NodeProps) {
  const d = data as unknown as PolicyData;
  return (
    <BaseNode nodeId={id} nodeType={NodeType.Policy} label={d.label} status={d.status} credence={d.credence} posterior={d.posterior} selected={selected}>
      {d.scope && (
