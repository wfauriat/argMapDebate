...
export default function UnderminesEdge(props: EdgeProps) {
  ...
  return (
    <>
      ...
      <EdgeLabelRenderer>
        <div
          className={`absolute text-[10px] font-medium bg-white/80 dark:bg-gray-800/80 px-1 rounded pointer-events-none nodrag nopan ${isWeakest ? "text-amber-600" : "text-red-500"}`}
          style={{
            transform: `translate(-50%, -50%) translate(${labelX}px, ${labelY}px)`,
          }}
        >
          Undermines{weightLabel}
        </div>
      </EdgeLabelRenderer>
    </>
  );
